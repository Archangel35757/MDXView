...
// read in a model using main engine code, then parse any extra stuff that this MDXView apps wants to know about...
//
		...
		if (hModel)
		{
			...
			switch (modtype)
			{
				...
				default:
					//assert(0);
					bModelOk = false;
					ErrorBox(va("The model \"%s\" is valid, but MDXView doesn't fully support this type at present",psLocalFilename));
					break;
			}
...
{
	if (gMDXViewTreeViewHandle)	// will be valid unless this is called from app exit
	{
		gMDXViewTreeViewHandle->DeleteAllItems();
	}
}
...
DWORD ModelTree_GetItemData(HTREEITEM hTreeItem)
{
	if (gMDXViewTreeViewHandle)	// will be valid unless this is called from app exit
	{
		return gMDXViewTreeViewHandle->GetTreeCtrl().GetItemData(hTreeItem);
	}
...
bool ModelTree_SetItemText(HTREEITEM hTreeItem, LPCSTR psText)
{
	if (gMDXViewTreeViewHandle)	// will be valid unless this is called from app exit
	{
		return !!gMDXViewTreeViewHandle->GetTreeCtrl().SetItemText(hTreeItem, psText);
	}
...
LPCSTR ModelTree_GetItemText(HTREEITEM hTreeItem, bool bPure /* = false */)
{
	if (gMDXViewTreeViewHandle)	// will be valid unless this is called from app exit
	{
		...
		static CString	string;
						string = gMDXViewTreeViewHandle->GetTreeCtrl().GetItemText(hTreeItem);
...
int ModelTree_GetChildCount(HTREEITEM hTreeItem)
{
	...
	if (gMDXViewTreeViewHandle)	// will be valid unless this is called from app exit	 
	{		
		if (gMDXViewTreeViewHandle->GetTreeCtrl().ItemHasChildren(hTreeItem))
		{
			hTreeItem = gMDXViewTreeViewHandle->GetTreeCtrl().GetChildItem(hTreeItem);			
...
bool ModelTree_ItemHasChildren(HTREEITEM hTreeItem)
{
	if (gMDXViewTreeViewHandle)	// will be valid unless this is called from app exit
	{
		return !!gMDXViewTreeViewHandle->GetTreeCtrl().ItemHasChildren(hTreeItem);
	}
...
HTREEITEM ModelTree_GetChildItem(HTREEITEM hTreeItem)
{
	if (gMDXViewTreeViewHandle)	// will be valid unless this is called from app exit
	{
		return gMDXViewTreeViewHandle->GetTreeCtrl().GetChildItem(hTreeItem);
	}
...
HTREEITEM ModelTree_GetNextSiblingItem(HTREEITEM hTreeItem)
{
	if (gMDXViewTreeViewHandle)	// will be valid unless this is called from app exit
	{
		return gMDXViewTreeViewHandle->GetTreeCtrl().GetNextSiblingItem(hTreeItem);
	}
...
HTREEITEM ModelTree_GetRootItem(void)
{
	if (gMDXViewTreeViewHandle)	// will be valid unless this is called from app exit
	{
		return gMDXViewTreeViewHandle->GetRootItem();
	}
...
bool ModelTree_DeleteItem(HTREEITEM hTreeItem)
{
	if (gMDXViewTreeViewHandle)	// will be valid unless this is called from app exit
	{
		return !!gMDXViewTreeViewHandle->GetTreeCtrl().DeleteItem(hTreeItem);
	}
...
{
	if (gMDXViewTreeViewHandle && Model_Loaded(hModel))	// will be valid unless this is called from app exit
	{
...
void ModelTree_InsertSequences(ModelContainer_t *pContainer, HTREEITEM hTreeItem_Sequences)
{
	...
	//
	HTREEITEM hTreeItemChild = gMDXViewTreeViewHandle->GetTreeCtrl().GetChildItem(hTreeItem_Sequences);
	if (hTreeItemChild)
	{
		while (hTreeItemChild)
		{
			HTREEITEM hNext = gMDXViewTreeViewHandle->GetTreeCtrl().GetNextSiblingItem(hTreeItemChild);

			gMDXViewTreeViewHandle->GetTreeCtrl().DeleteItem(hTreeItemChild);
...
HTREEITEM ModelTree_InsertItem(LPCTSTR psName, HTREEITEM hParent, UINT32 uiUserData /* = NULL */ , HTREEITEM hInsertAfter /* = TVI_LAST */)
{
	if (gMDXViewTreeViewHandle)	// will be valid unless this is called from app exit
	{
		return gMDXViewTreeViewHandle->InsertItem(psName, hParent, uiUserData, hInsertAfter);
	}
...
void App_OnceOnly(void)
{
	...
	OnceOnlyRefImport();	// init some rubbish that cut/paste code wants for other formats
...
static void ModelContainer_CallBack_AddToDrawList(ModelContainer_t* pContainer, void *pvData)
{
	glPushMatrix();
	{
		...
		if (bProceed)
		{
			R_MDXView_BeginEntityAdd();	// ##################

			// updateme?
			R_MDXView_AddEntity(pContainer->hModel,		// ModelHandle_t hModel,
